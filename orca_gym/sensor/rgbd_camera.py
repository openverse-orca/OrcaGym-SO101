...
class CameraCacher:
    ...
    async def do_stuff(self):
        ...
        async with websockets.connect(uri) as websocket:
            ...
            print(f"[CAM][WS] Connected to camera server: {uri}")
...
class Monitor:
    def __init__(self, name : str, fps=30, port=7070):
        ...
        self.fps = fps
        self.interval = 1.0 / self.fps  # 更新间隔，单位为秒
        self.running = False
        self.window_name = f"Camera Monitor - {name} (Port {port})"
    
    def start(self):
        """使用OpenCV显示相机画面（避免Qt依赖问题）"""
        self.running = True
        print(f"[Monitor] Starting camera monitor for {self.window_name}")
        print(f"[Monitor] Press 'q' or ESC to quit")
        
        try:
            while self.running:
                frame, _ = self.camera.get_frame(format='bgr24')  # OpenCV使用BGR格式
                
                if frame is not None:
                    # 在图像上显示信息
                    info_text = f"Frame: {self.camera.image_index}, FPS: {self.fps}"
                    ...
                    if key == ord('q') or key == 27:  # 'q' 或 ESC
                        print(f"[Monitor] Quit key pressed")
                        break
                else:
                    print(f"[Monitor] Warning: No frame received")
                    time.sleep(0.1)
                    
        except KeyboardInterrupt:
            print(f"[Monitor] Interrupted by user")
        finally:
            self.stop()
    ...
    def stop(self):
        ...
        cv2.destroyAllWindows()
        self.camera.stop()
        print(f"[Monitor] Monitor stopped")
