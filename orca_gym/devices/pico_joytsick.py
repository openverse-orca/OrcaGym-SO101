...
class PicoJoystick:
    def __init__(self, port=8001):
        ...
        self.port = port
        self._server_started = threading.Event()  # 用于通知主线程服务器是否启动成功
        self._server_error = None                  # 记录启动失败的异常
        self.thread = threading.Thread(target=self._start_server_thread, daemon=True)
        self.thread.start()
        # 等待最多 3 秒，确认服务器真正绑定成功
        if not self._server_started.wait(timeout=3.0):
            raise RuntimeError(
                f"PicoJoystick TCP 服务器启动超时（端口 {port}）。"
                f" 请检查端口是否已被占用：lsof -i:{port}"
            )
        if self._server_error is not None:
            raise RuntimeError(
                f"PicoJoystick TCP 服务器启动失败（端口 {port}）：{self._server_error}\n"
                f"请先释放端口：kill $(lsof -t -i:{port})"
    ...
    def _start_server_thread(self):
        ...
        try:
            ...
            self._server_started.set()  # 通知主线程（失败）
    ...
    async def _start_server(self):
        try:
            ...
            self._server_started.set()  # 通知主线程（失败）
            return
        self._server_started.set()  # 通知主线程（成功）
        async with server:
