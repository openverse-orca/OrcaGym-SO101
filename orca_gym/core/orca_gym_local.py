...
class OrcaGymLocal(OrcaGymBase):
    ...
    async def get_frame_png(self, image_path):
        ...
        t0 = time.time()
        _logger.info(f"[LoadLocalEnv/XML_FILE_NAME] send req_type={int(request.req_type)}")
        try:
            response = await self.stub.LoadLocalEnv(request, timeout=5.0)
        except grpc.aio.AioRpcError as e:
            _logger.error(
                f"[LoadLocalEnv/XML_FILE_NAME] RPC fail code={e.code()} details={e.details()} dt_ms={(time.time()-t0)*1000:.1f}"
            )
            raise
        _logger.info(
            f"[LoadLocalEnv/XML_FILE_NAME] recv status={response.status} file_name='{getattr(response,'file_name','')}' "
            f"error='{getattr(response,'error_message','')}' dt_ms={(time.time()-t0)*1000:.1f}"
        )
    ...
    #     t1 = time.time()
    #     _logger.info(f"[LoadLocalEnv/XML_FILE_CONTENT] send req_type={int(request.req_type)}")
    #     try:
    #         response = await self.stub.LoadLocalEnv(request, timeout=20.0)
    #     except grpc.aio.AioRpcError as e:
    #         _logger.error(
    #             f"[LoadLocalEnv/XML_FILE_CONTENT] RPC fail code={e.code()} details={e.details()} dt_ms={(time.time()-t1)*1000:.1f}"
    #         )
    #         raise
    #     _logger.info(
    #         f"[LoadLocalEnv/XML_FILE_CONTENT] recv status={response.status} xml_bytes={len(getattr(response,'xml_content',b''))} "
    #         f"error='{getattr(response,'error_message','')}' dt_ms={(time.time()-t1)*1000:.1f}"
    #     )


    #     if response.status != mjc_message_pb2.LoadLocalEnvResponse.SUCCESS:
    #         _logger.error(
    #             f"[LoadLocalEnv/XML_FILE_CONTENT] server returned failure status={response.status} "
    ...
    #             # if response.status != mjc_message_pb2.LoadLocalEnvResponse.SUCCESS:
    #             #     _logger.error(
    #             #         f"[LoadLocalEnv/XML_FILE_NAME] server returned failure status={response.status} "
    #             #         f"error_message='{response.error_message}'"
    #             #     )
    ...
    #                 # 只处理 MuJoCo 未初始化的情况
    #                 if (
    #                     response.status == 1 and
    #                     "MuJoCo has not been initialized" in response.error_message
    #                 ):
    #                     _logger.warning(
    #                         f"[LoadLocalEnv] MuJoCo not ready yet, retry {attempt+1}/{MAX_RETRY} "
    #                         f"after {RETRY_SLEEP_SEC}s"
    #                     )
    ...
    #                 # 其他错误：直接抛出，保持原语义
    #                 raise Exception(
    #                     f"Load local env failed. status={response.status}, "
    #                     f"error_message={response.error_message}"
    #                 )
    #             else:
    #                 # retry 用尽
    #                 raise Exception(
    #                     "Load local env failed: MuJoCo not ready after retries."
    #                 )
    ...
    async def load_local_env(self):
        ...
        for attempt in range(MAX_RETRY):
            try:
                ...
                _logger.error(
                    f"[LoadLocalEnv/XML_FILE_NAME] RPC fail code={e.code()} "
                    f"details={e.details()} dt_ms={(time.time()-t1)*1000:.1f}"
                )
                raise

            _logger.info(
                f"[LoadLocalEnv/XML_FILE_NAME] recv status={response.status} "
                f"file_name='{getattr(response,'file_name','')}' "
                f"error='{getattr(response,'error_message','')}' "
                f"dt_ms={(time.time()-t1)*1000:.1f}"
            )

            if response.status == mjc_message_pb2.LoadLocalEnvResponse.SUCCESS:
                break

            # 只对 MuJoCo 尚未初始化的情况做 retry
            if (
                response.status == 1 and
                "MuJoCo has not been initialized" in response.error_message
            ):
                _logger.warning(
                    f"[LoadLocalEnv/XML_FILE_NAME] MuJoCo not ready yet, "
                    f"retry {attempt+1}/{MAX_RETRY} after {RETRY_SLEEP_SEC}s"
                )
                await asyncio.sleep(RETRY_SLEEP_SEC)
                continue
            ...
            # 其他错误：保持原语义，直接失败
            _logger.error(
                f"[LoadLocalEnv/XML_FILE_NAME] server returned failure "
                f"status={response.status} error_message='{response.error_message}'"
            )
            raise Exception("Load local env failed.")
        ...
        # === 到这里，response 一定是 SUCCESS，file_name 一定存在 ===
        file_name = response.file_name
        file_path = os.path.join(self.xml_file_dir, file_name)
        ...
        async with file_lock(file_path):
            ...
            if not os.path.exists(file_path):
                ...
                t2 = time.time()
                _logger.info(
                    f"[LoadLocalEnv/XML_FILE_CONTENT] send req_type={int(request.req_type)} "
                ...
                _logger.info(
                    f"[LoadLocalEnv/XML_FILE_CONTENT] recv status={response.status} "
                    f"xml_bytes={len(getattr(response,'xml_content', b''))} "
                    f"error='{getattr(response,'error_message','')}' "
                    f"dt_ms={(time.time()-t2)*1000:.1f}"
                )

                if response.status != mjc_message_pb2.LoadLocalEnvResponse.SUCCESS:
                    _logger.error(
                        f"[LoadLocalEnv/XML_FILE_CONTENT] server returned failure "
                        f"status={response.status} error_message='{response.error_message}'"
                    )
                    raise Exception("Load local env failed.")
    ...
    async def set_timestep_remote(self, timestep):
        ...
        try:
            ...
            _logger.error(f"[SetOptTimestep] RPC failed: code={e.code()} details={e.details()}")
            raise
        # 视proto字段而定：success/status/error_message
        if hasattr(response, "error_message") and response.error_message:
            _logger.error(f"[SetOptTimestep] server error: {response.error_message}")
        return response
    ...
    def query_body_xpos_xmat_xquat(self, body_name_list):
        ...
        for body_name in body_name_list:
            # [DEBUG COMMENTED] 注释掉body查询的调试输出，着重查看reward相关信息
            # print("\n[DBG][orca_gym_local.query_body_xpos_xmat_xquat]")
            # print("  requested body_name =", body_name)
            # print("  all model bodies (head 20) =",
            #     list(self.model._body_dict.keys())[:20])
            # print("====================================================\n")
    ...
    def set_joint_qpos(self, joint_qpos):
        for joint_name, qpos in joint_qpos.items():
            # print("[DEBUG] joimy_name:",joint_name)  # 调试输出已关闭
            joint_id = self._mjModel.joint(joint_name).id
