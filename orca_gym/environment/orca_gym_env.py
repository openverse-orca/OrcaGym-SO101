...
class OrcaGymBaseEnv(gym.Env[NDArray[np.float64], NDArray[np.float32]]):
    ...
    def _resolve_name(self, kind: str, name: str, candidate: str) -> str:
        # 兼容 env.joint("") 这类用法：要拿到前缀本身
        if name == "":
            return candidate

        try:
            m = self.model  # OrcaGymModel
            if m is None:
                return candidate

            if kind == "joint":
                keys = m.get_joint_dict().keys()
            elif kind == "body":
                keys = m.get_body_names()
            elif kind == "site":
                keys = m.get_site_dict().keys()
            elif kind == "actuator":
                keys = m.get_actuator_dict().keys()
            else:
                return candidate
        ...
        return candidate  # 最后兜底，保持原行为便于暴露错误

    def _name_with_agent0(self, name: str, kind: str) -> str:
        if len(self._agent_names) == 0:
            return name
        cand = f"{self._agent_names[0]}_{name}"
        return self._resolve_name(kind, name, cand)
    
    def _name_with_agent(self, agent_id: int, name: str, kind: str | None = None) -> str:
       
        if len(self._agent_names) > 0:
            prefixed = f"{self._agent_names[agent_id]}_{name}"
            if kind is None:
                return prefixed
            return self._resolve_name(kind, name, prefixed)
        else:
            return name

    def body(self, name: str, agent_id=None) -> str:
        if agent_id is None:
            return self._name_with_agent0(name, "body")
        return self._name_with_agent(agent_id, name)

    def joint(self, name: str, agent_id=None) -> str:
        if agent_id is None:
            return self._name_with_agent0(name, "joint")
        return self._name_with_agent(agent_id, name)

    def site(self, name: str, agent_id=None) -> str:
        if agent_id is None:
            return self._name_with_agent0(name, "site")
        return self._name_with_agent(agent_id, name)

    def actuator(self, name: str, agent_id=None) -> str:
        if agent_id is None:
            return self._name_with_agent0(name, "actuator")
        return self._name_with_agent(agent_id, name)
