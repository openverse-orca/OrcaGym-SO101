...
class OrcaGymRemoteEnv(OrcaGymBaseEnv):
    ...
    def do_simulation(self, ctrl, n_frames) -> None:
        ...
        # 🔍 关键调试：打印接收到的ctrl
        if not hasattr(self, '_do_sim_counter'):
            self._do_sim_counter = 0
        self._do_sim_counter += 1
        if self._do_sim_counter % 20 == 0:
            print(f"\n🔧🔧🔧 [DO_SIMULATION] Step {self._do_sim_counter}: ctrl norm={np.linalg.norm(ctrl):.2f}, 范围[{np.min(ctrl):.1f}, {np.max(ctrl):.1f}]", flush=True)
    ...
    def _step_orca_sim_simulation(self, ctrl, n_frames):
        ...
        # 调试：每200步打印一次发送到仿真后端的ctrl
        if not hasattr(self, '_sim_debug_counter'):
            self._sim_debug_counter = 0
        self._sim_debug_counter += 1
        if self._sim_debug_counter % 200 == 0:
            print(f"[仿真后端] Step {self._sim_debug_counter}: 发送ctrl, norm={np.linalg.norm(ctrl):.2f}, 范围[{np.min(ctrl):.1f}, {np.max(ctrl):.1f}]")
