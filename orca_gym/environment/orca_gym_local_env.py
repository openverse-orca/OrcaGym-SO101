...
class OrcaGymLocalEnv(OrcaGymBaseEnv):
    ...
    def do_simulation(self, ctrl, n_frames) -> None:
        ...
        # 🔍 关键调试：打印接收到的ctrl（本地环境）
        # 减少日志输出频率：每500步输出一次（约20秒）
        if not hasattr(self, '_do_sim_counter'):
            self._do_sim_counter = 0
        self._do_sim_counter += 1
        if self._do_sim_counter % 500 == 0:
            print(f"\n🔧 [DO_SIMULATION-LOCAL] Step {self._do_sim_counter}: ctrl norm={np.linalg.norm(ctrl):.2f}, 范围[{np.min(ctrl):.1f}, {np.max(ctrl):.1f}]", flush=True)
