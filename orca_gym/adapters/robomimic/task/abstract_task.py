...
class AbstractTask:
    ...
    def _resolve_body_name(self, env, name: str) -> str:
        """
        把 task 里配置的 object/goal 名解析成 MuJoCo 里真实的 body 名。
        典型：blueblock -> blueblock_block
            redblock_usda -> redblock_usda_block
        """
        # 1) 候选命名规则（按常见程度排序）
        candidates = [
            name,
        ...
        # 2) 如果是 Local 模式，可以直接用 mjModel 的 body(name) 判断是否存在
        gym = getattr(env, "gym", None)
        model = getattr(gym, "_mjModel", None)
        if model is not None:
            for cand in candidates:
                try:
                    model.body(cand).id  # 存在则不会抛 KeyError
                    return cand
                except KeyError:
                    pass

        # 3) 兜底：先返回最可能的规则，方便你继续看日志
        return f"{name}_block"


    def get_object_xpos_xmat_xquat(self, env: OrcaGymLocalEnv):
        env_objects_body = [self._resolve_body_name(env, body_name) for body_name in self.object_bodys]
        print("\n[DBG][abstract_task.resolve_object_body]")
        print(f"  raw object_bodys = {self.object_bodys}")
        print(f"  resolved bodies  = {env_objects_body}")
        print("====================================================\n")
        return env.get_body_xpos_xmat_xquat(env_objects_body)


    def get_goal_xpos_xmat_xquat(self, env: OrcaGymLocalEnv):
        env_goals_body = [self._resolve_body_name(env, body_name) for body_name in self.goal_bodys]
        print("\n[DBG][abstract_task.resolve_goal_body]")
        print(f"  raw goal_bodys   = {self.goal_bodys}")
        print(f"  resolved bodies  = {env_goals_body}")
        print("====================================================\n")
        return env.get_body_xpos_xmat_xquat(env_goals_body)

    # def get_object_xpos_xmat_xquat(self, env: OrcaGymLocalEnv):
    #     env_objects_body = [env.body(body_name) for body_name in self.object_bodys]
    #     # ================== DEBUG: object body name tracing ==================
    #     print("\n[DBG][abstract_task.get_object_xpos_xmat_xquat]")
    #     print("  env_objects_body =", env_objects_body)

    #     try:
    #         print("  self.objects =", getattr(self, "objects", None))
    #     except Exception as e:
    #         print("  self.objects <print failed>:", e)

    #     try:
    #         print("  env.objects =", getattr(env, "objects", None))
    #     except Exception as e:
    #         print("  env.objects <print failed>:", e)

    #     try:
    #         print("  self.__dict__.keys() =", list(self.__dict__.keys()))
    #     except Exception:
    #         pass
    #     print("================================================================\n")
    #     # ====================================================================
    ...
    def generate_object(self, env:OrcaGymLocalEnv, pick_min, pick_max):
        ...
        if self.random_actor:
            if self.__random_count__ % self.random_cycle == 0:
                ...
                self.object_joints = [f"{self.actors[idx]}__joint_0" for idx in idxs]
    ...
    def generate_actors(self):
        '''
        将所有的 actors 添加到场景中, 初始化到 infinity 位置
        '''
        for i in range(len(self.actors)):
            actor_name = self.actors[i]
            asset_path = self.actors_spawnable[i]
            # actor_name = "blueblock"
            # asset_path = "assets/prefabs/blueblock_usda"

            print(
                    "[NEW_DEBUG][generate_actors] try add_actor:",
                    "actor_name =", actor_name,
                    "asset_path =", asset_path,
                    "len:",len(self.actors),
                    "spawnable:",self.actors_spawnable,

                )

            try:
                print(
                    "[DEBUG][generate_actors] try add_actor:",
                    "actor_name =", actor_name,
                    "asset_path =", asset_path,
                    flush=True
                )
                ...
                print(
                    "[DEBUG][generate_actors] add_actor SUCCESS:",
                    actor_name,
                    flush=True
                )
                ...
                print(
                    "\n[ERROR][generate_actors] add_actor FAILED",
                    "\n  actor_name =", actor_name,
                    "\n  asset_path =", asset_path,
                    "\n  exception  =", repr(e),
                    flush=True
