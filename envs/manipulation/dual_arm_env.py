...
class DualArmEnv(RobomimicEnv):
    ...
    def step(self, action) -> tuple:
        ...
        if DEBUG_REWARD:
            print(f"\n[REWARD_DEBUG] DualArmEnv.step() called, _run_mode={self._run_mode}")
        ...
        # ========== 🔍 调试输出：捕获 do_simulation 前的控制信号 ==========
        # 每20步打印一次（使用计数器，避免浮点数精度问题）
        if self._debug_step_counter % 20 == 0:
            print(f"\n\n{'#'*70}", flush=True)
            print(f"### 🔍 [DO_SIMULATION前] Step={self._debug_step_counter:04d}, 模式={self._run_mode}", flush=True)
            print(f"{'#'*70}", flush=True)
            print(f"📊 ctrl统计:", flush=True)
            print(f"   范围: [{np.min(ctrl):.2f}, {np.max(ctrl):.2f}]", flush=True)
            print(f"   Norm: {np.linalg.norm(ctrl):.2f}", flush=True)
            print(f"   前5值: {ctrl[:5]}", flush=True)
            
            if self._run_mode == RunMode.POLICY_NORMALIZED:
                # Inference 模式：打印动作中的位姿信息
                print(f"\n🤖 noscaled_action (来自INFERENCE):", flush=True)
                print(f"   左臂位置(B): {noscaled_action[:3]}", flush=True)
                print(f"   左臂姿态(轴): {noscaled_action[3:6]}", flush=True)
                print(f"   左爪: {noscaled_action[13]:.3f}", flush=True)
                print(f"   右臂位置(B): {noscaled_action[14:17]}", flush=True)
                print(f"   右臂姿态(轴): {noscaled_action[17:20]}", flush=True)
                print(f"   右爪: {noscaled_action[27]:.3f}", flush=True)
            elif self._run_mode == RunMode.TELEOPERATION:
                # Xbox 模式：打印从手柄读取的位姿
                print(f"\n🎮 noscaled_action (来自XBOX):", flush=True)
                print(f"   左臂位置(B): {noscaled_action[:3]}", flush=True)
                print(f"   左臂姿态(轴): {noscaled_action[3:6]}", flush=True)
            print(f"{'#'*70}\n\n", flush=True)
        # ========== 调试输出结束 ==========
        ...
        # 仿真完成后打印夹爪接触力/角度日志（与 ctrl 和 q 同帧，数据准确）
        if self._run_mode == RunMode.TELEOPERATION:
            for agent in self._agents.values():
    ...
    def _compute_reward_obj_goal_distance(self) -> float:
        # [MODIFIED] tracking_sigma 从 0.025 -> 1.0 (提升40倍)
        # 原因：任务距离尺度是 1.3m+，原值 0.025 导致 exp(-75) ≈ 0（数值下溢）
        # 新值 1.0 使得：distance=1m -> reward≈0.37, distance=1.4m -> reward≈0.14
        tracking_sigma = 1.0  # 原: 0.025
        ...
        if DEBUG_REWARD: print(f"[REWARD_DEBUG] obj_goal_distance: obj_pos={obj_xpos}, goal_pos={goal_xpos}, "
              f"distance={distance:.4f}, distance_squared={distance_squared:.6f}, reward={reward:.6f}")
        
        return reward
    
    def _compute_reward_obj_grasp_distance(self) -> float:
        # [MODIFIED] tracking_sigma 从 0.025 -> 1.0 (提升40倍)
        # 原因：末端执行器到物体的距离也是 1.2m+，原值导致奖励为 0
        tracking_sigma = 1.0  # 原: 0.025
        ...
        if DEBUG_REWARD: print(f"[REWARD_DEBUG] obj_grasp_distance: obj_pos={obj_xpos}, ee_pos={ee_xpos}, "
              f"distance={distance:.4f}, distance_squared={distance_squared:.6f}, reward={reward:.6f}")
    ...
    def _compute_reward_success(self) -> float:
        ...
        if DEBUG_REWARD: print(f"[REWARD_DEBUG] success: is_success={success}, reward={reward}")
    ...
    def _setup_reward_functions(self, reward_type : RewardType) -> None:
        if reward_type == RewardType.SPARSE:
            ...
            # [MODIFIED] 增加系数从 0.1 -> 1.0，使奖励对距离变化更敏感（提升10倍）
            # 原因：object 和 goal 距离较远（1.3m+），原系数 0.1 导致奖励过小
            self._reward_functions = [
                {"function": self._compute_reward_obj_goal_distance, "coeff": 1.0},  # 原: 0.1
                {"function": self._compute_reward_obj_grasp_distance, "coeff": 1.0},  # 原: 0.1
                {"function": self._compute_reward_success, "coeff": 1.0},
            ]
        else:
    ...
    def _compute_reward(self, info) -> float:
        if DEBUG_REWARD:
            print(f"\n[REWARD_DEBUG] _compute_reward called")
            if DEBUG_REWARD: print(f"[REWARD_DEBUG] info keys: {list(info.keys())}")
            if DEBUG_REWARD: print(f"[REWARD_DEBUG] reward_functions: {[f['function'].__name__ for f in self._reward_functions]}")
        
        # 从info中提取object和goal信息，设置到self._obs中供reward函数使用
        import json
        self._obs = {}
        
        if "object" in info and info["object"] is not None:
            if isinstance(info["object"], str):
                # 解析JSON字符串
                obj_dict = json.loads(info["object"])
                # 获取第一个object（取字典的第一个值）
                obj_entry = list(obj_dict.values())[0]
                obj_pos = np.array(obj_entry["position"][:3], dtype=np.float32)  # 只取前3个值（x, y, z）
                obj_quat = np.array(obj_entry["orientation"], dtype=np.float32)
                self._obs["object"] = np.concatenate([obj_pos, obj_quat])
                if DEBUG_REWARD: print(f"[REWARD_DEBUG] object position: {obj_pos}")
            elif isinstance(info["object"], np.ndarray) and len(info["object"]) > 0:
                obj_entry = info["object"][0]
                obj_pos = np.array(obj_entry["position"], dtype=np.float32)
                obj_quat = np.array(obj_entry["orientation"], dtype=np.float32)
                self._obs["object"] = np.concatenate([obj_pos, obj_quat])
                if DEBUG_REWARD: print(f"[REWARD_DEBUG] object position: {obj_pos}")
            else:
                self._obs["object"] = np.zeros(7, dtype=np.float32)
                if DEBUG_REWARD: print(f"[REWARD_DEBUG] object set to zeros (unknown format)")
        else:
            self._obs["object"] = np.zeros(7, dtype=np.float32)
            if DEBUG_REWARD: print(f"[REWARD_DEBUG] object set to zeros (not in info or None)")
        
        if "goal" in info and info["goal"] is not None:
            if isinstance(info["goal"], str):
                # 解析JSON字符串
                goal_dict = json.loads(info["goal"])
                # 获取第一个goal（取字典的第一个值）
                goal_entry = list(goal_dict.values())[0]
                goal_pos = np.array(goal_entry["position"][:3], dtype=np.float32)  # 只取前3个值（x, y, z）
                goal_quat = np.array(goal_entry["orientation"], dtype=np.float32)
                self._obs["goal"] = np.concatenate([goal_pos, goal_quat])
                if DEBUG_REWARD: print(f"[REWARD_DEBUG] goal position: {goal_pos}")
            elif isinstance(info["goal"], np.ndarray) and len(info["goal"]) > 0:
                goal_entry = info["goal"][0]
                goal_pos = np.array(goal_entry["position"], dtype=np.float32)
                goal_quat = np.array(goal_entry["orientation"], dtype=np.float32)
                self._obs["goal"] = np.concatenate([goal_pos, goal_quat])
                if DEBUG_REWARD: print(f"[REWARD_DEBUG] goal position: {goal_pos}")
            else:
                self._obs["goal"] = np.zeros(7, dtype=np.float32)
                if DEBUG_REWARD: print(f"[REWARD_DEBUG] goal set to zeros (unknown format)")
        else:
            self._obs["goal"] = np.zeros(7, dtype=np.float32)
            if DEBUG_REWARD: print(f"[REWARD_DEBUG] goal set to zeros (not in info or None)")
        
        # 获取末端执行器位置（从agent的obs中获取）
        if len(self._agents) > 0:
            agent_obs = self._agents[list(self._agents.keys())[0]].get_obs()
            if "ee_pos_l" in agent_obs and "ee_pos_r" in agent_obs:
                # 使用左臂的末端执行器位置
                self._obs["ee_pos"] = agent_obs["ee_pos_l"]
                if DEBUG_REWARD: print(f"[REWARD_DEBUG] ee_pos (left arm): {agent_obs['ee_pos_l']}")
            elif "ee_pos" in agent_obs:
                self._obs["ee_pos"] = agent_obs["ee_pos"]
                if DEBUG_REWARD: print(f"[REWARD_DEBUG] ee_pos: {agent_obs['ee_pos']}")
            else:
                self._obs["ee_pos"] = np.zeros(3, dtype=np.float32)
        else:
            self._obs["ee_pos"] = np.zeros(3, dtype=np.float32)
        ...
        for reward_function in self._reward_functions:
            if reward_function["coeff"] == 0:
                ...
                raw_reward = reward_function["function"]()
                weighted_reward = raw_reward * reward_function["coeff"]
                total_reward += weighted_reward
                if DEBUG_REWARD: print(f"[REWARD_DEBUG] {reward_function['function'].__name__}: raw={raw_reward:.6f}, "
                      f"coeff={reward_function['coeff']}, weighted={weighted_reward:.6f}")
                self._print_reward(reward_function["function"].__name__, weighted_reward, reward_function["coeff"])

        if DEBUG_REWARD: print(f"[REWARD_DEBUG] ===== Total Reward: {total_reward:.6f} =====\n")
